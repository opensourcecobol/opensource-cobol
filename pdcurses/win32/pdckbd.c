...
#define IP_BUFF_SIZE 80
static INPUT_RECORD save_ip;
static INPUT_RECORD save_ip_buf[IP_BUFF_SIZE];
...
/* SJIS Flags */
static int current_buffsize = 0;
static int restof_buffsize = 0;
static int current_byte_type = PDC_CHAR_NONE;
...
int PDC_get_key(void)
{
    ...
    if (!key_count)
    {
		...
		if (restof_buffsize == 0)
		{
			ReadConsoleInput(pdc_con_in, save_ip_buf, IP_BUFF_SIZE, &count);
			event_count--;
			restof_buffsize = count;
			current_buffsize = count;
			current_byte_type = PDC_CHAR_ANSII;
		}
		save_ip = save_ip_buf[current_buffsize - restof_buffsize];
		if (current_byte_type == PDC_CHAR_SJIS_HIGH){
			current_byte_type = PDC_CHAR_SJIS_LOW;
		}
		else {
			if (PDC_IS_SJIS_HIGH(save_ip.Event.KeyEvent.uChar.AsciiChar)){
				current_byte_type = PDC_CHAR_SJIS_HIGH;
			}
			else {
				current_byte_type = PDC_CHAR_ANSII;
			}
...
void PDC_clear_current_buffer(void)
{
	restof_buffsize = 0;
	current_byte_type = PDC_CHAR_NONE;
}

int PDC_get_restof_buffsize(void)
{
	return restof_buffsize;
}

int PDC_get_current_byte_type(void)
{
	return current_byte_type;
}
